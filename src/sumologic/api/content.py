...
class ContentManagementApi:
    ...
    def sync(self, folder_id, content, overwrite):
        '''
        Import content inside the folder with the given folder_id.
        '''
        # submit the job
        base_uri = "v2/content/folders/{0}/import".format(folder_id)
        queryParams =  {'overwrite': overwrite}
        sync_job = self.api_client.post(base_uri, params=queryParams, data=content).json()
